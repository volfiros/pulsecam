interface ROIOverlayProps {
  rois: { x: number; y: number; width: number; height: number }[] | null;
  videoWidth: number;
  videoHeight: number;
}

export default function ROIOverlay({ rois, videoWidth, videoHeight }: ROIOverlayProps) {
  if (!rois || videoWidth === 0 || videoHeight === 0) return null;
  ...
  return (
    <>
      {rois.map((roi, i) => (
        <div
          key={i}
          className="fixed border-2 rounded-sm pointer-events-none"
          style={{
